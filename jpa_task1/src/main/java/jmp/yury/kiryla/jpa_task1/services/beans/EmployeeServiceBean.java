...
public class EmployeeServiceBean implements EmployeeService {
	...
	Employee managedEmployee = find(employee.getId());
	if (employee != null) {
	    EntityTransaction tx = em.getTransaction();
	    tx.begin();
	    managedEmployee.setAddress(employee.getAddress());
	    managedEmployee.setPersonalInfo(employee.getPersonalInfo());
	    managedEmployee.setStatus(employee.getStatus());
	    managedEmployee.setProjects(employee.getProjects());
	    managedEmployee.setUnit(employee.getUnit());
	    tx.commit();
	}
    }
}
